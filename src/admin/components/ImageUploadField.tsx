...
type Props = {
  ...
  compact?: boolean
  /** Show button to pick an existing image from the media library. Default true. */
...
export function ImageUploadField({
  ...
  async function onPick(files: FileList | null) {
    ...
    const { publicUrl, error } = await uploadCmsImage(supabase, folder, f)
    setBusy(false)
    if (error || !publicUrl) {
        ...
        {value ? (
          <div className="overflow-hidden rounded-2xl border border-ink/10 bg-white ring-1 ring-ink/5">
            <img
              src={value}
              alt=""
              className="h-24 w-full max-w-[200px] object-cover sm:h-28"
            />
          </div>
        ) : null}
